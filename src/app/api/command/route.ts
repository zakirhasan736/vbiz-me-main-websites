...
Provide a friendly text response acknowledging the action.`,
        ...
        responseSchema: {
          ...
          properties: {
            action: {
              type: Type.STRING,
              description:
                'The parsed action (CALL, EMAIL, OPEN_VIDEO, SEARCH_WEB, OPEN_NOTEPAD, UNKNOWN)',
            },
            target: {
              type: Type.STRING,
              description:
                "The name, email, or search query extracted from the command. For example, if it says 'EMAIL TO mc', target is 'mc'. If 'OPEN mc INTRO VIDEOS', target is 'mc intro videos'.",
            },
            responseMessage: {
              type: Type.STRING,
              description:
                "A friendly, conversational response summarizing what you are doing. e.g. 'Opening intro videos for mc...'",
            },
          },
    ...
    const parsedCommand = JSON.parse(resultText) as Record<string, unknown>;
